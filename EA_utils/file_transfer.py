def from_local_to_sharepoint(top_source_directory_path, top_destination_directory_path, source_and_destination_subdirectories, objects_not_to_copy, paths_not_to_copy, replace_or_update_or_keep_or_forbid_existing_duplicate_files):

    #usage example:
    # import EA_utils.file_transfer as file_transfer
    #####################################################################################################
    ...
    #####################################################################################################
    # file_transfer.from_local_to_sharepoint(top_source_directory_path, top_destination_directory_path, source_and_destination_subdirectories, objects_not_to_copy, paths_not_to_copy, replace_or_update_or_keep_or_forbid_existing_duplicate_files)
    ...
    elapsed_time = et - st
    if elapsed_time < 60:
        print('\nExecution time:', "{:.2f}".format(elapsed_time), 'seconds')
    elif elapsed_time >= 60 and elapsed_time < 60*60:
        print('\nExecution time:', "{:.2f}".format(elapsed_time/60), 'minutes')
    else:
        print('\nExecution time:', "{:.2f}".format(elapsed_time/60/60), 'hours')
